...
def run_bot():
    while True: # uses previous functions to get input, split it into its lemmetised words, then predict & return a response
        ...
        if accuracy < ERROR_THRESHOLD:
            ...
            res = google_definition(message) # returns google / wikipedia answer if accuracy too low
            return "<p>" + res + "</p>" # returns result
        else:
            res = get_response(predicted_class) # gets result
            return "<p>" + res + "</p>" # returns result
